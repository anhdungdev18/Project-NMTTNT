...
class GameState:
    ...
    # sua
    def copy(self):
        # Tạo một bản sao của GameState
        new_state = GameState()
        new_state.board = [row[:] for row in self.board]  # Sao chép bàn cờ
    ...
    def makeMove(self, move):
        ...
        if move.is_pawn_promotion:
            if self.white_to_move:
                ...
                promoted_piece = self.showPromotionDialog()
    ...
    def showPromotionDialog(self):
        """
        Hiển thị giao diện chọn quân cờ phong cấp trên màn hình chính mà không thay đổi kích thước.
        """
        pygame.init()
        promotion_pieces = [("Queen", "Q"), ("Rook", "R"), ("Bishop", "B"), ("Knight", "N")]
        POPUP_WIDTH = 300
        POPUP_HEIGHT = 400
        BUTTON_WIDTH = 260
        BUTTON_HEIGHT = 60
    ...
    # Màu sắc
        ...
        HOVER_COLOR = (200, 200, 200)  # Màu xám khi hover
        BORDER_COLOR = (100, 100, 100)  # Màu viền xám đậm
        BUTTON_COLOR = (230, 230, 230)
        TEXT_COLOR = (0, 0, 0)
        BUTTON_HOVER_COLOR = HOVER_COLOR

    # Create surfaces once, not every frame
    ...
    # Pre-render text surfaces
        title = title_font.render("Promote to", True, TEXT_COLOR)
        title_rect = title.get_rect(centerx=POPUP_WIDTH//2, top=PADDING)
    ...
    # Pre-calculate positions
        ...
        buttons = []
        for i, (piece_name, piece_symbol) in enumerate(promotion_pieces):
            button_rect = pygame.Rect(
                (POPUP_WIDTH - BUTTON_WIDTH) // 2,  # Căn giữa ngang
                title_rect.bottom + PADDING + i * (BUTTON_HEIGHT + PADDING),  # Vị trí dọc với khoảng cách
                BUTTON_WIDTH,
                BUTTON_HEIGHT
            )
            buttons.append((button_rect, piece_symbol, piece_name))

        # Pre-render button texts
        button_texts = {}
        for piece_name, _ in promotion_pieces:
            button_texts[piece_name] = font.render(piece_name, True, TEXT_COLOR)
        ...
        while True:
            ...
            relative_mouse_pos = (mouse_pos[0] - popup_x, mouse_pos[1] - popup_y)
        
            for button_rect, _, piece_name in buttons:
                color = HOVER_COLOR if button_rect.collidepoint(relative_mouse_pos) else BUTTON_COLOR
                pygame.draw.rect(popup, BORDER_COLOR, button_rect)
                pygame.draw.rect(popup, color, button_rect.inflate(-4, -4))
                text = button_texts[piece_name]
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    for button_rect, piece_symbol, _ in buttons:
